...
`2.1.0` is a CPU-only stable API foundation for the tensor, autograd,
...
- TensorFlow-inspired hardware diagnostics for backend descriptors, physical
  device properties, logical devices, placement, execution plans, and transfer
  routes, plus native storage allocation telemetry.
- NumPy copy interop, pickle-based internal serialization, non-pickle NPZ
  tensor/state_dict files, limited ONNX export, optional ONNX Runtime
  inference for compatible static graphs, vision IO, transforms, checksummed
  image manifests, datasets, metrics, visualization, compact CNN helpers,
  examples, tests, benchmarks, and GitHub Actions release workflows.
...
graph compilation, grouped convolution, adaptive/global pooling, complete NumPy
advanced-indexing parity, sparse tensors, mixed precision, a high-performance
kernel library, pretrained vision model zoo, detection/segmentation training
stack, video IO, a bundled ONNX runtime, or ONNX import. Benchmarks are rough
...
- `interchange`: NumPy, serialization, trusted pickle notes, ONNX export,
  metadata/runtime diagnostics, and formats.
