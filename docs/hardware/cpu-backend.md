...
TensorStudio `2.1.0` supports CPU tensors only, with a stronger native runtime
...
- Native storage telemetry records checkout counts, active bytes, cumulative
  bytes, and peak active usage for diagnostics.
...
CUDA and Metal execution are future work. The `2.1.0` backend boundary defines
device descriptors, backend availability metadata, allocator/runtime/logical
device descriptors, transfer and execution planning, descriptor-only plugin
kernel manifests, CMake hooks, and device-aware storage validation. Real
accelerator execution still requires:
